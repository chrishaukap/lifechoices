...
class Gl20Renderer implements GLSurfaceView.Renderer {
    ...
    private float mRate = 0.0125f;
    private void adjustBGColor()
    {
    	if(whichColor ==1) {
    		if((mRed += mRate) > 1.0f) {mRed = 1.0f; whichColor = 2;}
         if((mGreen -= mRate) < 0.0f) mGreen = 0.0f;
         if((mBlue -= mRate) < 0.0f) mBlue = 0.0f;
    	}
    	if(whichColor ==2){
    	   if((mGreen += mRate) > 1.0f) {mGreen = 1.0f; whichColor = 3;}
         if((mRed -= mRate) < 0.0f) mRed = 0.0f;
         if((mBlue -= mRate) < 0.0f) mBlue = 0.0f;
      }
    	if(whichColor ==3){
    	   if((mBlue += mRate) > 1.0f) {mBlue = 1.0f; whichColor = 1;}
    	   if((mGreen -= mRate) < 0.0f) mGreen = 0.0f;
    	   if((mRed -= mRate) < 0.0f) mRed = 0.0f;
      }   	
    }
